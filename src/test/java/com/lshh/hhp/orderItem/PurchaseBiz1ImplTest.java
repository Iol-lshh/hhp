...
import com.lshh.hhp.orderItem.service.OrderItemServiceImpl;
...
class PurchaseBiz1ImplTest {
    ...
    @InjectMocks
    OrderItemServiceImpl purchaseService;

    OrderItemDto purchase;
    RequestPurchaseDto requestPurchaseDto;
    List<RequestPurchaseDto> requestPurchaseList;

    @BeforeEach
    public void setup() {
        purchase = new OrderItemDto()
                .userId(1L)
                .orderId(1L)
                .productId(1L)
                .paid(100)
    ...
    void purchase() throws Exception {
        ...
        // Act
        List<OrderItemDto> result = purchaseService.purchase(purchase.userId(), purchase.orderId(), Collections.singletonList(requestPurchaseDto));

        // Assert
        verify(purchaseComponent).save(any());
        verify(pointComponent).subtract(any());
